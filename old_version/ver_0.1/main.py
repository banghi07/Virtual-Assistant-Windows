...
class assistant():
    ...
    def request(self, text):
        if "none" in text:
            ...
            pass
        elif "có thể làm gì" in text or "trợ giúp" in text or "help" in text:
            self.helpMe()
        elif "chào" in text or "hello" in text:
            self.greeting()
        elif "hôm nay" in text or "bữa nay" in text or "nay" in text or "hiện tại" in text \
              or "bây giờ" in text or "ngày mấy" in text or "mấy giờ" in text:
            self.getTime(text) 
        elif "mở nhạc" in text or "phát nhạc" in text or "chơi nhạc" in text \
              or "mở video" in text or "mở mv" in text:
              self.playSong()
        elif "thời tiết" in text or "thời tiết hôm nay" in text:
            self.whatWeather()
        elif "tin tức" in text or "thời sự" in text:
            self.readNews()
    ...
    def searchWiki(self, text):
        ...
        result = {
            "content": content,
            "string": "Đây là kết quả tìm kiếm được từ Wikipedia"
        }
    ...
    def searchThread(self, text):
        ...
        worker.signals.finished.connect(self.searchDefaultComplete)
        self.threadpool.start(worker)
        self.setPlainTextEdit("Đang tìm kiếm. Vui lòng đợi...")

    def search(self, text):
        driver = webdriver.Edge(EdgeChromiumDriverManager().install())
        # driver = webdriver.Edge("D:/Projects/Virtual Assistant/edgedriver_win64/msedgedriver.exe")
        # browser = webdriver.Edge("D:\Projects\Virtual Assistant\edgedriver_win64\msedgedriver.exe")
        driver.get("https://www.google.com")
        searchBox = driver.find_element(By.NAME, "q")
        searchBox.send_keys(text + Keys.RETURN)

    def searchDefaultComplete(self):
        self.response("Đây là kết quả tìm kiếm của bạn.")
